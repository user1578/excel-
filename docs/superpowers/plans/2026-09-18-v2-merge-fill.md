...
- 初始基线为 149 passed；每个阶段必须运行 python -m pytest -q，所有新测试使用 tmp_path 或 tests/data 的虚构数据。
...
- Consumes: ImportService.apply_mappings() 的 StandardField.DATE 映射。
...
保留 read_raw() 的 DataFrame 返回以免破坏其它调用点；增加供 ImportService 使用的上下文读取函数。xlsx 上下文只标记原单元格具有日期 number format 且可取得 workbook epoch 的数值单元格。CSV 与无元数据输入不提供日期序列上下文。
...
保留/补齐纵向字段并集、学号优先、姓名加班级、唯一姓名、重名不自动合并、空值加非空、冲突和来源追溯测试。

- [ ] **Step 2: 运行测试确认失败**

Run: python -m pytest tests/test_dataset_merge.py tests/test_ui_smoke.py -q

Expected: FAIL，因为当前导出路径由服务生成，页面没有保存对话框。

- [ ] **Step 3: 实现显式路径与页面状态**

MergeExportService 验证 .xlsx 后缀、未解决冲突的明确许可、保存并重开目标。MergePage 使用 QFileDialog.getSaveFileName，默认建议 exports/资料汇总.xlsx，取消时不调用服务。渲染来源文件数、原始行数、最终人数、匹配数、未匹配数、冲突数、缺失数，并通过全部/有冲突/缺失/无法匹配筛选预览，不改变 MergeResult。

- [ ] **Step 4: 验证并提交**

Run: python -m pytest tests/test_dataset_merge.py tests/test_ui_smoke.py -q

Expected: PASS，导出保留 汇总结果、汇总说明、字段冲突 工作表与安全文本。

~~~bash
git add app/services/dataset_merge_service.py app/models/merge_models.py app/services/merge_export_service.py app/ui/merge_page.py tests/test_dataset_merge.py tests/test_ui_smoke.py
git commit -m "feat: 完善资料汇总预览与另存为"
~~~

### Task 4: 原子输出与 .xlsx 兼容填充（阶段 D）
...
**Interfaces:**
- Produces: AtomicWorkbookOutput(source_path, output_path)，提供 temporary_path、commit(validate) 和 cleanup()。
...
~~~python
with AtomicWorkbookOutput(analysis.template_path, output_path) as transaction:
...
临时文件必须在目标文件的父目录且保持 .xlsx 后缀。commit 仅在完整保存和 reopen 成功后 os.replace 到最终路径。异常分支 close workbook、删除临时文件并原样抛出；最终文件事前存在时只有成功时才替换。
...
### Task 5: COM 默认引擎与无 Excel 降级（阶段 E）
...
**Interfaces:**
- Produces: ExcelComService.is_available()、analyze(path, sheet_name, header_row)、fill(..., output_path)。
...
另加同后缀临时副本、Save 而非 FileFormat=51 转换、异常清理和 .xlsx 兼容模式明确同意测试。
...
finally 中关闭 workbook 和 Excel。临时文件后缀必须等于源/最终后缀。COM 分析 Sheet/表头以支持 .xls/.xlsx/.xlsm；禁止 SaveAs(FileFormat=51) 或任何后缀变化；win32com 仅在 is_available/fill 时导入。

- [ ] **Step 4: 接入 UI 同意与另存为**

选择模板后由 coordinator 说明实际引擎。无 Excel + .xlsx 时，显示本次操作 openpyxl 兼容模式无法保证任意高级 Office 对象的明确警示；仅 Yes 设置 compatibility_accepted。无 Excel + .xls/.xlsm 直接说明拒绝原因。最终路径来自 QFileDialog.getSaveFileName，默认 <stem>_已填写<suffix>；取消或源路径相同都不能写入。
...
### Task 6: 模板 Schema、照片和外部另存为（阶段 F）
...
在 requirements.txt 添加 Pillow；用 Pillow 读取尺寸，以 min(max_width/width, max_height/height, 1) 缩放，锚定目标单元格且不承诺居中。复用该受限图片帮助器到 TemplateGenerator 和兼容 WorkbookFillService。ExcelComService 使用 Excel 的 Shapes/AddPicture API，设置 LockAspectRatio，并将形状限制于目标单元格区域。TemplateManager 保存 schema.json 供管理页加载/复制/删除；最终用户 xlsx 通过 AtomicWorkbookOutput 保存到 UI 选择位置，不能强制放到 templates。非图片用户值应用 safe_excel_value()，系统公式保持原样。
...
Expected: 应用启动且无 import error。若当前环境不能显示 GUI，运行离屏 UI 测试并明确记录该限制，不把它称为真实启动验证。
...
~~~bash
git add README.md tests/test_ui_smoke.py
